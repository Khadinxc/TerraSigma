resource "azurerm_sentinel_alert_rule_scheduled" "command_executed_via_run_dialog_box_registry" {
  ...
  description                = "Detects execution of commands via the run dialog box on Windows by checking values of the \"RunMRU\" registry key. This technique was seen being abused by threat actors to deceive users into pasting and executing malicious commands, often disguised as CAPTCHA verification steps. - Likely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/registry/registry_set/command_executed_via_run_dialog_box_registry.tf"
  severity                   = "Low"
  query                      = <<QUERY
