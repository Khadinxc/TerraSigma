resource "azurerm_sentinel_alert_rule_scheduled" "webdav_temporary_local_file_creation" {
  ...
  description                = "Detects the creation of WebDAV temporary files with potentially suspicious extensions - Legitimate use of WebDAV in an environment | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/file/file_event/webdav_temporary_local_file_creation.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceFileEvents
