resource "azurerm_sentinel_alert_rule_scheduled" "file_access_win_office_outlook_mail_credential" {
  name                       = "file_access_win_office_outlook_mail_credential"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Access To Windows Outlook Mail Files By Uncommon Applications"
  description                = "Detects file access requests to Windows Outlook Mail by uncommon processes. Could indicate potential attempt of credential stealing. Requires heavy baselining before usage Reference: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/file/file_access/file_access_win_office_outlook_mail_credential.yml - Antivirus, Anti-Spyware, Anti-Malware Software - Backup software - Legitimate software installed on partitions other than \"C:\\\" - Searching software such as \"everything.exe\" | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/file/file_access/file_access_win_office_outlook_mail_credential.yml"
  severity                   = "Low"
  query                      = <<QUERY
DeviceFileEvents
