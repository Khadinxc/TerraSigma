resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_powershell_abnormal_commandline_size" {
  ...
  description                = <<DESC
    ...
    Reference: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/process_creation/proc_creation_win_powershell_abnormal_commandline_size.yml
  DESC
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
