resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_susp_ntfs_short_name_path_use_cli" {
  name                       = "proc_creation_win_susp_ntfs_short_name_path_use_cli"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Use Short Name Path in Command Line"
  description                = "Detects the use of short name paths (8.3 format) in command lines, which can be used to obfuscate paths or access restricted locations. Windows creates short 8.3 filenames (like PROGRA~1) for compatibility with MS-DOS-based or 16-bit Windows programs. When investigating, examine: - Commands using short paths to access sensitive directories or files - Web servers on Windows (especially Apache) where short filenames could bypass security controls - Correlation with other suspicious behaviors - baseline of short name usage in your environment and look for deviations Reference: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/process_creation/proc_creation_win_susp_ntfs_short_name_path_use_cli.yml - Applications could use this notation occasionally which might generate some false positives. In that case investigate the parent and child process. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/process_creation/proc_creation_win_susp_ntfs_short_name_path_use_cli.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
