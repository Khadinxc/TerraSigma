resource "azurerm_sentinel_alert_rule_scheduled" "diskshadow_child_process_spawned" {
  ...
  description                = "Detects any child process spawning from \"Diskshadow.exe\". This could be due to executing Diskshadow in interpreter mode or script mode and using the \"exec\" flag to launch other applications. - Likely from legitimate usage of Diskshadow in Interpreter mode. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/process_creation/diskshadow_child_process_spawned.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
