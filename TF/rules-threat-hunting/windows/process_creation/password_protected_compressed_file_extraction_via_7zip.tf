resource "azurerm_sentinel_alert_rule_scheduled" "password_protected_compressed_file_extraction_via_7zip" {
  ...
  description                = "Detects usage of 7zip utilities (7z.exe, 7za.exe and 7zr.exe) to extract password protected zip files. - Legitimate activity is expected since extracting files with a password can be common in some environment. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/process_creation/password_protected_compressed_file_extraction_via_7zip.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
