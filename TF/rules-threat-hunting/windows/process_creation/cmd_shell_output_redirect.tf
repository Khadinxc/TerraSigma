resource "azurerm_sentinel_alert_rule_scheduled" "cmd_shell_output_redirect" {
  ...
  description                = "Detects the use of the redirection character \">\" to redirect information on the command line. This technique is sometimes used by malicious actors in order to redirect the output of reconnaissance commands such as \"hostname\" and \"dir\" to files for future exfiltration. - Internet Download Manager extensions use named pipes and redirection via CLI. Filter it out if you use it in your environment | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-threat-hunting/windows/process_creation/cmd_shell_output_redirect.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
