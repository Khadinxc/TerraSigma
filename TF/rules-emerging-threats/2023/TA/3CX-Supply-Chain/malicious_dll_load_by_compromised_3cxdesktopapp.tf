resource "azurerm_sentinel_alert_rule_scheduled" "malicious_dll_load_by_compromised_3cxdesktopapp" {
  ...
  description                = "Detects DLL load activity of known compromised DLLs used in by the compromised 3CXDesktopApp - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-emerging-threats/2023/TA/3CX-Supply-Chain/malicious_dll_load_by_compromised_3cxdesktopapp.tf"
  severity                   = "High"
  query                      = <<QUERY
