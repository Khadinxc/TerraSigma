resource "azurerm_sentinel_alert_rule_scheduled" "lace_tempest_cobalt_strike_download" {
  ...
  description                = "Detects specific command line execution used by Lace Tempest to download Cobalt Strike as reported by SysAid Team - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-emerging-threats/2023/TA/Lace-Tempest/lace_tempest_cobalt_strike_download.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
