resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_apt_ta17_293a_ps" {
  ...
  query                      = <<QUERY
DeviceProcessEvents
| where ProcessCommandLine contains "ps.exe -accepteula" and ProcessCommandLine contains "-s cmd /c netstat"
QUERY
