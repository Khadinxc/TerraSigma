resource "azurerm_sentinel_alert_rule_scheduled" "winnti_pipemon_characteristics" {
  ...
  description                = "Detects specific process characteristics of Winnti Pipemon malware reported by ESET - Legitimate setups that use similar flags | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-emerging-threats/2020/TA/Winnti/winnti_pipemon_characteristics.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
