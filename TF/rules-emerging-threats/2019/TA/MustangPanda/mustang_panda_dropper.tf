resource "azurerm_sentinel_alert_rule_scheduled" "mustang_panda_dropper" {
  ...
  description                = "Detects specific process parameters as used by Mustang Panda droppers - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-emerging-threats/2019/TA/MustangPanda/mustang_panda_dropper.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
