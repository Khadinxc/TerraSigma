resource "azurerm_sentinel_alert_rule_scheduled" "forest_blizzard_apt_custom_protocol_handler_creation" {
  ...
  description                = "Detects the setting of a custom protocol handler with the name \"rogue\". Seen being created by Forest Blizzard APT as reported by MSFT. - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-emerging-threats/2024/TA/Forest-Blizzard/forest_blizzard_apt_custom_protocol_handler_creation.tf"
  severity                   = "High"
  query                      = <<QUERY
