resource "azurerm_sentinel_alert_rule_scheduled" "forest_blizzard_apt_process_creation_activity" {
  ...
  description                = "Detects the execution of specific processes and command line combination. These were seen being created by Forest Blizzard as described by MSFT. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules-emerging-threats/2024/TA/Forest-Blizzard/forest_blizzard_apt_process_creation_activity.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
