resource "azurerm_sentinel_alert_rule_scheduled" "indicator_removal_on_host_clear_mac_system_logs" {
  name                       = "indicator_removal_on_host_clear_mac_system_logs"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Indicator Removal on Host - Clear Mac System Logs"
  description                = "Detects deletion of local audit logs - Legitimate administration activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/indicator_removal_on_host_clear_mac_system_logs.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
