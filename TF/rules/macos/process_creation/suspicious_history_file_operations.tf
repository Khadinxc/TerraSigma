resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_history_file_operations" {
  ...
  description                = "Detects commandline operations on shell history files - Legitimate administrative activity - Legitimate software, cleaning hist file | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/suspicious_history_file_operations.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
