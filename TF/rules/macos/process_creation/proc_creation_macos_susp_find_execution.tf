resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_macos_susp_find_execution" {
  name                       = "proc_creation_macos_susp_find_execution"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Potential Discovery Activity Using Find - MacOS"
  description                = "Detects usage of \"find\" binary in a suspicious manner to perform discovery Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/proc_creation_macos_susp_find_execution.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/proc_creation_macos_susp_find_execution.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
