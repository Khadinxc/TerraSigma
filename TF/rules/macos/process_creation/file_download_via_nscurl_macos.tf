resource "azurerm_sentinel_alert_rule_scheduled" "file_download_via_nscurl_macos" {
  ...
  description                = "Detects the execution of the nscurl utility in order to download files. - Legitimate usage of nscurl by administrators and users. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/file_download_via_nscurl_macos.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
