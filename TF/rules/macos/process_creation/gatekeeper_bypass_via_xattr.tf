resource "azurerm_sentinel_alert_rule_scheduled" "gatekeeper_bypass_via_xattr" {
  ...
  description                = "Detects macOS Gatekeeper bypass via xattr utility - Legitimate activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/gatekeeper_bypass_via_xattr.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
