resource "azurerm_sentinel_alert_rule_scheduled" "credentials_from_password_stores_keychain" {
  ...
  description                = "Detects passwords dumps from Keychain - Legitimate administration activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/credentials_from_password_stores_keychain.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
