resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_macos_remote_access_tools_renamed_meshagent_execution" {
  ...
  description                = <<DESC
    ...
    Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/macos/process_creation/proc_creation_macos_remote_access_tools_renamed_meshagent_execution.yml
  DESC
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
