resource "azurerm_sentinel_alert_rule_scheduled" "eventlog_evtx_file_deleted" {
  ...
  description                = "Detects the deletion of the event log files which may indicate an attempt to destroy forensic evidence | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_delete/eventlog_evtx_file_deleted.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceFileEvents
