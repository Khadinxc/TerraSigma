resource "azurerm_sentinel_alert_rule_scheduled" "windows_shell_scripting_application_file_write_to_suspicious_folder" {
  ...
  description                = "Detects Windows shells and scripting applications that write files to suspicious folders | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/windows_shell_scripting_application_file_write_to_suspicious_folder.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
