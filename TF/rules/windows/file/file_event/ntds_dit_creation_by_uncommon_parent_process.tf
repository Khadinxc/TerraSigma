resource "azurerm_sentinel_alert_rule_scheduled" "ntds_dit_creation_by_uncommon_parent_process" {
  ...
  description                = "Detects creation of a file named \"ntds.dit\" (Active Directory Database) by an uncommon parent process or directory | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/ntds_dit_creation_by_uncommon_parent_process.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
