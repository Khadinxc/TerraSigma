resource "azurerm_sentinel_alert_rule_scheduled" "hacktool_mimikatz_kirbi_file_creation" {
  ...
  description                = "Detects the creation of files created by mimikatz such as \".kirbi\", \"mimilsa.log\", etc. - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/hacktool_mimikatz_kirbi_file_creation.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
