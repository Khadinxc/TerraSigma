resource "azurerm_sentinel_alert_rule_scheduled" "file_event_win_ntds_dit_uncommon_process" {
  name                       = "file_event_win_ntds_dit_uncommon_process"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "NTDS.DIT Creation By Uncommon Process"
  description                = "Detects creation of a file named \"ntds.dit\" (Active Directory Database) by an uncommon process or a process located in a suspicious directory Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/file_event_win_ntds_dit_uncommon_process.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/file_event_win_ntds_dit_uncommon_process.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
