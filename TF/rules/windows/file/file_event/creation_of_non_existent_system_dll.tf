resource "azurerm_sentinel_alert_rule_scheduled" "creation_of_non_existent_system_dll" {
  ...
  description                = "Detects the creation of system DLLs that are usually not present on the system (or at least not in system directories). Usually this technique is used to achieve DLL hijacking. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/creation_of_non_existent_system_dll.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceFileEvents
