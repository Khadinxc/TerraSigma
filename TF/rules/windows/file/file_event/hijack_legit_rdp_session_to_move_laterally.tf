resource "azurerm_sentinel_alert_rule_scheduled" "hijack_legit_rdp_session_to_move_laterally" {
  ...
  description                = "Detects the usage of tsclient share to place a backdoor on the RDP source machine's startup folder - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/hijack_legit_rdp_session_to_move_laterally.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
