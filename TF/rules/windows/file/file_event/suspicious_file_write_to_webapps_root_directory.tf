resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_file_write_to_webapps_root_directory" {
  ...
  description                = "Detects suspicious file writes to the root directory of web applications, particularly Apache web servers or Tomcat servers. This may indicate an attempt to deploy malicious files such as web shells or other unauthorized scripts. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/suspicious_file_write_to_webapps_root_directory.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceFileEvents
