resource "azurerm_sentinel_alert_rule_scheduled" "file_event_win_susp_legitimate_app_dropping_exe" {
  ...
  description                = "Detects programs on a Windows system that should not write executables to disk Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/file_event_win_susp_legitimate_app_dropping_exe.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/file_event_win_susp_legitimate_app_dropping_exe.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
