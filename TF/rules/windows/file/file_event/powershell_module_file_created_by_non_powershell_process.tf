resource "azurerm_sentinel_alert_rule_scheduled" "powershell_module_file_created_by_non_powershell_process" {
  ...
  description                = "Detects the creation of a new PowerShell module \".psm1\", \".psd1\", \".dll\", \".ps1\", etc. by a non-PowerShell process | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/powershell_module_file_created_by_non_powershell_process.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceFileEvents
