resource "azurerm_sentinel_alert_rule_scheduled" "psexec_remote_execution_file_artefact" {
  ...
  description                = "Detects creation of the PSEXEC key file. Which is created anytime a PsExec command is executed. It gets written to the file system and will be recorded in the USN Journal on the target system - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/psexec_remote_execution_file_artefact.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
