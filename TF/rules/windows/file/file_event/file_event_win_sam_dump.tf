resource "azurerm_sentinel_alert_rule_scheduled" "file_event_win_sam_dump" {
  name                       = "file_event_win_sam_dump"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Potential SAM Database Dump"
  description                = "Detects the creation of files that look like exports of the local SAM (Security Account Manager) Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/file_event_win_sam_dump.yml - Rare cases of administrative activity | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/file_event_win_sam_dump.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
