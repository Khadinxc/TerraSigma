resource "azurerm_sentinel_alert_rule_scheduled" "potential_persistence_via_outlook_form" {
  ...
  description                = "Detects the creation of a new Outlook form which can contain malicious code - Legitimate use of outlook forms | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/file/file_event/potential_persistence_via_outlook_form.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
