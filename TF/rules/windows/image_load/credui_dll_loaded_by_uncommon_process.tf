resource "azurerm_sentinel_alert_rule_scheduled" "credui_dll_loaded_by_uncommon_process" {
  ...
  description                = "Detects loading of \"credui.dll\" and related DLLs by an uncommon process. Attackers might leverage this DLL for potential use of \"CredUIPromptForCredentials\" or \"CredUnPackAuthenticationBufferW\". - Other legitimate processes loading those DLLs in your environment. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/credui_dll_loaded_by_uncommon_process.tf"
  severity                   = "Medium"
  query                      = <<QUERY
