resource "azurerm_sentinel_alert_rule_scheduled" "powershell_core_dll_loaded_by_non_powershell_process" {
  ...
  description                = "Detects loading of essential DLLs used by PowerShell by non-PowerShell process. Detects behavior similar to meterpreter's \"load powershell\" extension. - Used by some .NET binaries, minimal on user workstation. - Used by Microsoft SQL Server Management Studio | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/powershell_core_dll_loaded_by_non_powershell_process.tf"
  severity                   = "Medium"
  query                      = <<QUERY
