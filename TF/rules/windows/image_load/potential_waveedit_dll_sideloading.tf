resource "azurerm_sentinel_alert_rule_scheduled" "potential_waveedit_dll_sideloading" {
  ...
  description                = "Detects potential DLL sideloading of \"waveedit.dll\", which is part of the Nero WaveEditor audio editing software. - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/potential_waveedit_dll_sideloading.tf"
  severity                   = "High"
  query                      = <<QUERY
