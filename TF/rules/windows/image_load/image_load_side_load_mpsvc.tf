resource "azurerm_sentinel_alert_rule_scheduled" "image_load_side_load_mpsvc" {
  ...
  description                = "Detects potential DLL sideloading of \"MpSvc.dll\". Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/image_load_side_load_mpsvc.yml - Legitimate applications loading their own versions of the DLL mentioned in this rule. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/image_load_side_load_mpsvc.yml"
  severity                   = "Medium"
  query                      = <<QUERY
