resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_volume_shadow_copy_vssapi_dll_load" {
  ...
  description                = "Detects the image load of VSS DLL by uncommon executables | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/suspicious_volume_shadow_copy_vssapi_dll_load.tf"
  severity                   = "High"
  query                      = <<QUERY
