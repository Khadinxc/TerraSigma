resource "azurerm_sentinel_alert_rule_scheduled" "wmiprvse_wbemcomn_dll_hijack" {
  ...
  description                = "Detects a threat actor creating a file named `wbemcomn.dll` in the `C:\\Windows\\System32\\wbem\\` directory over the network and loading it for a WMI DLL Hijack scenario. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/wmiprvse_wbemcomn_dll_hijack.tf"
  severity                   = "High"
  query                      = <<QUERY
