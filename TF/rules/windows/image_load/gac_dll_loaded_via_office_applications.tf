resource "azurerm_sentinel_alert_rule_scheduled" "gac_dll_loaded_via_office_applications" {
  ...
  description                = "Detects any GAC DLL being loaded by an Office Product - Legitimate macro usage. Add the appropriate filter according to your environment | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/gac_dll_loaded_via_office_applications.tf"
  severity                   = "High"
  query                      = <<QUERY
