resource "azurerm_sentinel_alert_rule_scheduled" "diagnostic_library_sdiageng_dll_loaded_by_msdt_exe" {
  ...
  description                = "Detects both of CVE-2022-30190 (Follina) and DogWalk vulnerabilities exploiting msdt.exe binary to load the \"sdiageng.dll\" library | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/diagnostic_library_sdiageng_dll_loaded_by_msdt_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
