resource "azurerm_sentinel_alert_rule_scheduled" "potentially_suspicious_volume_shadow_copy_vsstrace_dll_load" {
  ...
  description                = "Detects the image load of VSS DLL by uncommon executables | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/potentially_suspicious_volume_shadow_copy_vsstrace_dll_load.tf"
  severity                   = "Medium"
  query                      = <<QUERY
