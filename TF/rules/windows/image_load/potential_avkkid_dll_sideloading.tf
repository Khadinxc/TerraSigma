resource "azurerm_sentinel_alert_rule_scheduled" "potential_avkkid_dll_sideloading" {
  ...
  description                = "Detects potential DLL sideloading of \"AVKkid.dll\" | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/potential_avkkid_dll_sideloading.tf"
  severity                   = "Medium"
  query                      = <<QUERY
