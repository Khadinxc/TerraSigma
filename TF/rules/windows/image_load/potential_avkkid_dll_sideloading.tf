resource "azurerm_sentinel_alert_rule_scheduled" "potential_avkkid_dll_sideloading" {
  name                       = "potential_avkkid_dll_sideloading"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Potential AVKkid.DLL Sideloading"
  description                = "Detects potential DLL sideloading of \"AVKkid.dll\" | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/potential_avkkid_dll_sideloading.yml"
  severity                   = "Medium"
  query                      = <<QUERY
