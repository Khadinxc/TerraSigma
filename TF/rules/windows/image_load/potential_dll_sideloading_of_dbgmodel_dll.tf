resource "azurerm_sentinel_alert_rule_scheduled" "potential_dll_sideloading_of_dbgmodel_dll" {
  ...
  description                = "Detects potential DLL sideloading of \"DbgModel.dll\" - Legitimate applications loading their own versions of the DLL mentioned in this rule | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/potential_dll_sideloading_of_dbgmodel_dll.tf"
  severity                   = "Medium"
  query                      = <<QUERY
