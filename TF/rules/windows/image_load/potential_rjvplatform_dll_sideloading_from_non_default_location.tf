resource "azurerm_sentinel_alert_rule_scheduled" "potential_rjvplatform_dll_sideloading_from_non_default_location" {
  ...
  description                = "Detects potential DLL sideloading of \"RjvPlatform.dll\" by \"SystemResetPlatform.exe\" located in a non-default location. - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/potential_rjvplatform_dll_sideloading_from_non_default_location.tf"
  severity                   = "High"
  query                      = <<QUERY
