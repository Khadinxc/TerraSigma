resource "azurerm_sentinel_alert_rule_scheduled" "potential_ccleanerdu_dll_sideloading" {
  ...
  description                = "Detects potential DLL sideloading of \"CCleanerDU.dll\" | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/image_load/potential_ccleanerdu_dll_sideloading.yml"
  severity                   = "Medium"
  query                      = <<QUERY
