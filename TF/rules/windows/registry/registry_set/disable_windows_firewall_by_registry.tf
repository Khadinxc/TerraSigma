resource "azurerm_sentinel_alert_rule_scheduled" "disable_windows_firewall_by_registry" {
  ...
  description                = "Detect set EnableFirewall to 0 to disable the Windows firewall | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/disable_windows_firewall_by_registry.tf"
  severity                   = "Medium"
  query                      = <<QUERY
