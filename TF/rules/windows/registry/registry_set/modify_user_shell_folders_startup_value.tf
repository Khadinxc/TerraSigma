resource "azurerm_sentinel_alert_rule_scheduled" "modify_user_shell_folders_startup_value" {
  ...
  description                = "Detect modification of the startup key to a path where a payload could be stored to be launched during startup | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/modify_user_shell_folders_startup_value.tf"
  severity                   = "High"
  query                      = <<QUERY
