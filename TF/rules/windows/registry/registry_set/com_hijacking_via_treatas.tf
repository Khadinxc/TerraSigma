resource "azurerm_sentinel_alert_rule_scheduled" "com_hijacking_via_treatas" {
  ...
  description                = "Detect modification of TreatAs key to enable \"rundll32.exe -sta\" command - Legitimate use | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/com_hijacking_via_treatas.tf"
  severity                   = "Medium"
  query                      = <<QUERY
