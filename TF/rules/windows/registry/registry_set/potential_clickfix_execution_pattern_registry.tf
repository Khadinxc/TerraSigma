resource "azurerm_sentinel_alert_rule_scheduled" "potential_clickfix_execution_pattern_registry" {
  ...
  description                = "Detects potential ClickFix malware execution patterns by monitoring registry modifications in RunMRU keys containing HTTP/HTTPS links. ClickFix is known to be distributed through phishing campaigns and uses techniques like clipboard hijacking and fake CAPTCHA pages. Through the fakecaptcha pages, the adversary tricks users into opening the Run dialog box and pasting clipboard-hijacked content, such as one-liners that execute remotely hosted malicious files or scripts. - Legitimate applications using RunMRU with HTTP links | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/potential_clickfix_execution_pattern_registry.yml"
  severity                   = "High"
  query                      = <<QUERY
