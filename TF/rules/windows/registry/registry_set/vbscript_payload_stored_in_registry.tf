resource "azurerm_sentinel_alert_rule_scheduled" "vbscript_payload_stored_in_registry" {
  ...
  description                = "Detects VBScript content stored into registry keys as seen being used by UNC2452 group | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/vbscript_payload_stored_in_registry.tf"
  severity                   = "High"
  query                      = <<QUERY
