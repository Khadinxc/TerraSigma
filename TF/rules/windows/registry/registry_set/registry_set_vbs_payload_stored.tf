resource "azurerm_sentinel_alert_rule_scheduled" "registry_set_vbs_payload_stored" {
  name                       = "registry_set_vbs_payload_stored"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "VBScript Payload Stored in Registry"
  description                = "Detects VBScript content stored into registry keys as seen being used by UNC2452 group Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/registry_set_vbs_payload_stored.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/registry_set_vbs_payload_stored.yml"
  severity                   = "High"
  query                      = <<QUERY
