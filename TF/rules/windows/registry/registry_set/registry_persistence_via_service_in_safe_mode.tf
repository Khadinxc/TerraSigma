resource "azurerm_sentinel_alert_rule_scheduled" "registry_persistence_via_service_in_safe_mode" {
  ...
  description                = "Detects the modification of the registry to allow a driver or service to persist in Safe Mode. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/registry_persistence_via_service_in_safe_mode.tf"
  severity                   = "High"
  query                      = <<QUERY
