resource "azurerm_sentinel_alert_rule_scheduled" "filefix_command_evidence_in_typedpaths" {
  ...
  description                = "Detects commonly-used chained commands and strings in the most recent 'url' value of the 'TypedPaths' key, which could be indicative of a user being targeted by the FileFix technique. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/filefix_command_evidence_in_typedpaths.tf"
  severity                   = "High"
  query                      = <<QUERY
