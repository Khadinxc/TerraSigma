resource "azurerm_sentinel_alert_rule_scheduled" "service_binary_in_suspicious_folder" {
  ...
  description                = "Detect the creation of a service with a service binary located in a suspicious directory | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/service_binary_in_suspicious_folder.tf"
  severity                   = "High"
  query                      = <<QUERY
