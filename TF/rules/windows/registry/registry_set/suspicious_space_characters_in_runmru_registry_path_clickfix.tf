resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_space_characters_in_runmru_registry_path_clickfix" {
  ...
  description                = "Detects the occurrence of numerous space characters in RunMRU registry paths, which may indicate execution via phishing lures using clickfix techniques to hide malicious commands in the Windows Run dialog box from naked eyes. - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/suspicious_space_characters_in_runmru_registry_path_clickfix.tf"
  severity                   = "High"
  query                      = <<QUERY
