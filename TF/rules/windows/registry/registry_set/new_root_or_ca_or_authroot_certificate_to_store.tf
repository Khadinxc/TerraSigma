resource "azurerm_sentinel_alert_rule_scheduled" "new_root_or_ca_or_authroot_certificate_to_store" {
  ...
  description                = "Detects the addition of new root, CA or AuthRoot certificates to the Windows registry | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/new_root_or_ca_or_authroot_certificate_to_store.tf"
  severity                   = "Medium"
  query                      = <<QUERY
