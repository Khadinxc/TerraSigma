resource "azurerm_sentinel_alert_rule_scheduled" "persistence_via_hhctrl_ocx" {
  ...
  description                = "Detects when an attacker modifies the registry value of the \"hhctrl\" to point to a custom binary - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_set/persistence_via_hhctrl_ocx.tf"
  severity                   = "High"
  query                      = <<QUERY
