resource "azurerm_sentinel_alert_rule_scheduled" "dll_load_via_lsass" {
  ...
  description                = "Detects a method to load DLL via LSASS process using an undocumented Registry key | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/registry/registry_event/dll_load_via_lsass.tf"
  severity                   = "High"
  query                      = <<QUERY
