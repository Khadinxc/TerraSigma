resource "azurerm_sentinel_alert_rule_scheduled" "network_connection_initiated_to_devtunnels_domain" {
  ...
  description                = "Detects network connections to Devtunnels domains initiated by a process on a system. Attackers can abuse that feature to establish a reverse shell or persistence on a machine. - Legitimate use of Devtunnels will also trigger this. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/network_connection/network_connection_initiated_to_devtunnels_domain.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceNetworkEvents
