resource "azurerm_sentinel_alert_rule_scheduled" "silenttrinity_stager_msbuild_activity" {
  ...
  description                = "Detects a possible remote connections to Silenttrinity c2 | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/network_connection/silenttrinity_stager_msbuild_activity.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceNetworkEvents
