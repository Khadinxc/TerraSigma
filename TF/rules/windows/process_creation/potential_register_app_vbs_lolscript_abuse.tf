resource "azurerm_sentinel_alert_rule_scheduled" "potential_register_app_vbs_lolscript_abuse" {
  ...
  description                = "Detects potential abuse of the \"register_app.vbs\" script that is part of the Windows SDK. The script offers the capability to register new VSS/VDS Provider as a COM+ application. Attackers can use this to install malicious DLLs for persistence and execution. - Other VB scripts that leverage the same starting command line flags | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/potential_register_app_vbs_lolscript_abuse.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
