resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_rundll32_parent_explorer" {
  ...
  description                = "Detects execution of \"rundll32.exe\" with a parent process of Explorer.exe. This has been observed by variants of Raspberry Robin, as first reported by Red Canary. Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_rundll32_parent_explorer.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_rundll32_parent_explorer.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
