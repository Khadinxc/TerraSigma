resource "azurerm_sentinel_alert_rule_scheduled" "use_icacls_to_hide_file_to_everyone" {
  ...
  description                = "Detect use of icacls to deny access for everyone in Users folder sometimes used to hide malicious files | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/use_icacls_to_hide_file_to_everyone.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
