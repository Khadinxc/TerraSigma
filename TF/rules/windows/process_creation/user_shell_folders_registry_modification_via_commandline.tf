resource "azurerm_sentinel_alert_rule_scheduled" "user_shell_folders_registry_modification_via_commandline" {
  ...
  description                = "Detects modifications to User Shell Folders registry values via reg.exe or PowerShell, which could indicate persistence attempts. Attackers may modify User Shell Folders registry values to point to malicious executables or scripts that will be executed during startup. This technique is often used to maintain persistence on a compromised system by ensuring that malicious payloads are executed automatically. - Usage of reg.exe or PowerShell to modify User Shell Folders for legitimate purposes; but rare. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/user_shell_folders_registry_modification_via_commandline.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
