resource "azurerm_sentinel_alert_rule_scheduled" "file_download_via_bitsadmin_to_an_uncommon_target_folder" {
  ...
  description                = "Detects usage of bitsadmin downloading a file to uncommon target folder | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/file_download_via_bitsadmin_to_an_uncommon_target_folder.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
