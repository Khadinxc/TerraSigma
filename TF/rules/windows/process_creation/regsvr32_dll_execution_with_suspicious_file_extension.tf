resource "azurerm_sentinel_alert_rule_scheduled" "regsvr32_dll_execution_with_suspicious_file_extension" {
  ...
  description                = "Detects the execution of REGSVR32.exe with DLL files masquerading as other files - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/regsvr32_dll_execution_with_suspicious_file_extension.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
