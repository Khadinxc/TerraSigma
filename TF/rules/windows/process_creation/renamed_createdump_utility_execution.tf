resource "azurerm_sentinel_alert_rule_scheduled" "renamed_createdump_utility_execution" {
  name                       = "renamed_createdump_utility_execution"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Renamed CreateDump Utility Execution"
  description                = "Detects uses of a renamed legitimate createdump.exe LOLOBIN utility to dump process memory - Command lines that use the same flags | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/renamed_createdump_utility_execution.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
