resource "azurerm_sentinel_alert_rule_scheduled" "insecure_proxy_doh_transfer_via_curl_exe" {
  ...
  description                = "Detects execution of \"curl.exe\" with the \"insecure\" flag over proxy or DOH. - Access to badly maintained internal or development systems | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/insecure_proxy_doh_transfer_via_curl_exe.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
