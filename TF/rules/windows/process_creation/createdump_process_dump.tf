resource "azurerm_sentinel_alert_rule_scheduled" "createdump_process_dump" {
  ...
  description                = "Detects uses of the createdump.exe LOLOBIN utility to dump process memory - Command lines that use the same flags | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/createdump_process_dump.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
