resource "azurerm_sentinel_alert_rule_scheduled" "pua_csexec_execution" {
  ...
  description                = "Detects the use of the lesser known remote execution tool named CsExec a PsExec alternative | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/pua_csexec_execution.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
