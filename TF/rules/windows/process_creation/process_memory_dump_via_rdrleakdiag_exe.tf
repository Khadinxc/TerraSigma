resource "azurerm_sentinel_alert_rule_scheduled" "process_memory_dump_via_rdrleakdiag_exe" {
  ...
  description                = "Detects the use of the Microsoft Windows Resource Leak Diagnostic tool \"rdrleakdiag.exe\" to dump process memory - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/process_memory_dump_via_rdrleakdiag_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
