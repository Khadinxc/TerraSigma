resource "azurerm_sentinel_alert_rule_scheduled" "data_copied_to_clipboard_via_clip_exe" {
  ...
  description                = "Detects the execution of clip.exe in order to copy data to the clipboard. Adversaries may collect data stored in the clipboard from users copying information within or between applications. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/data_copied_to_clipboard_via_clip_exe.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
