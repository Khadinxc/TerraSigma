resource "azurerm_sentinel_alert_rule_scheduled" "invoke_obfuscation_compress_obfuscation" {
  ...
  description                = "Detects Obfuscated Powershell via COMPRESS OBFUSCATION | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/invoke_obfuscation_compress_obfuscation.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
