resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_bcdedit_susp_execution" {
  ...
  description                = "Detects potential malicious and unauthorized usage of bcdedit.exe Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_bcdedit_susp_execution.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_bcdedit_susp_execution.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
