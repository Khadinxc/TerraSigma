resource "azurerm_sentinel_alert_rule_scheduled" "powershell_sam_copy" {
  ...
  description                = "Detects suspicious PowerShell scripts accessing SAM hives - Some rare backup scenarios - PowerShell scripts fixing HiveNightmare / SeriousSAM ACLs | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/powershell_sam_copy.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
