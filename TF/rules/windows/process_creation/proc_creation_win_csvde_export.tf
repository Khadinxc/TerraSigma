resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_csvde_export" {
  name                       = "proc_creation_win_csvde_export"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Active Directory Structure Export Via Csvde.EXE"
  description                = "Detects the execution of \"csvde.exe\" in order to export organizational Active Directory structure. Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_csvde_export.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_csvde_export.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
