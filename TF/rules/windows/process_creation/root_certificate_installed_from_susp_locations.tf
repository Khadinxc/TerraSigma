resource "azurerm_sentinel_alert_rule_scheduled" "root_certificate_installed_from_susp_locations" {
  ...
  description                = "Adversaries may install a root certificate on a compromised system to avoid warnings when connecting to adversary controlled web servers. - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/root_certificate_installed_from_susp_locations.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
