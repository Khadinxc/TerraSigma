resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_office_outlook_susp_child_processes" {
  name                       = "proc_creation_win_office_outlook_susp_child_processes"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Suspicious Outlook Child Process"
  description                = "Detects a suspicious process spawning from an Outlook process. Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_office_outlook_susp_child_processes.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_office_outlook_susp_child_processes.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
