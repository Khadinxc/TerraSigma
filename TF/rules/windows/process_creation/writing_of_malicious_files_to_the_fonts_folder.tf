resource "azurerm_sentinel_alert_rule_scheduled" "writing_of_malicious_files_to_the_fonts_folder" {
  ...
  description                = "Monitors for the hiding possible malicious files in the C:\\Windows\\Fonts\\ location. This folder doesn't require admin privillege to be written and executed from. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/writing_of_malicious_files_to_the_fonts_folder.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
