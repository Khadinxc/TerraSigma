resource "azurerm_sentinel_alert_rule_scheduled" "share_and_session_enumeration_using_net_exe" {
  ...
  description                = "Detects attempts to enumerate file shares, printer shares and sessions using \"net.exe\" with the \"view\" flag. - Legitimate use of net.exe utility by legitimate user | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/share_and_session_enumeration_using_net_exe.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
