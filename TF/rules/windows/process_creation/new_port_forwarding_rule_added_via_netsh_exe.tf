resource "azurerm_sentinel_alert_rule_scheduled" "new_port_forwarding_rule_added_via_netsh_exe" {
  name                       = "new_port_forwarding_rule_added_via_netsh_exe"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "New Port Forwarding Rule Added Via Netsh.EXE"
  description                = "Detects the execution of netsh commands that configure a new port forwarding (PortProxy) rule - Legitimate administration activity - WSL2 network bridge PowerShell script used for WSL/Kubernetes/Docker (e.g. https://github.com/microsoft/WSL/issues/4150#issuecomment-504209723) | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/new_port_forwarding_rule_added_via_netsh_exe.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
