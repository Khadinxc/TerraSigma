resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_mstsc_run_local_rpd_file_susp_parent" {
  name                       = "proc_creation_win_mstsc_run_local_rpd_file_susp_parent"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Mstsc.EXE Execution From Uncommon Parent"
  description                = "Detects potential RDP connection via Mstsc using a local \".rdp\" file located in suspicious locations. Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_mstsc_run_local_rpd_file_susp_parent.yml - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_mstsc_run_local_rpd_file_susp_parent.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
