resource "azurerm_sentinel_alert_rule_scheduled" "cscript_wscript_potentially_suspicious_child_process" {
  ...
  description                = "Detects potentially suspicious child processes of Wscript/Cscript. These include processes such as rundll32 with uncommon exports or PowerShell spawning rundll32 or regsvr32. Malware such as Pikabot and Qakbot were seen using similar techniques as well as many others. - Some false positives might occur with admin or third party software scripts. Investigate and apply additional filters accordingly. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/cscript_wscript_potentially_suspicious_child_process.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
