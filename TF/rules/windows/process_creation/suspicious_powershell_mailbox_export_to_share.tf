resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_powershell_mailbox_export_to_share" {
  ...
  description                = "Detects usage of the powerShell New-MailboxExportRequest Cmdlet to exports a mailbox to a remote or local share, as used in ProxyShell exploitations | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_powershell_mailbox_export_to_share.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
