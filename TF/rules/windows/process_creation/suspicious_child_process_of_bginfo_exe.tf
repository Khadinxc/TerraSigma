resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_child_process_of_bginfo_exe" {
  ...
  description                = "Detects suspicious child processes of \"BgInfo.exe\" which could be a sign of potential abuse of the binary to proxy execution via external VBScript | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_child_process_of_bginfo_exe.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
