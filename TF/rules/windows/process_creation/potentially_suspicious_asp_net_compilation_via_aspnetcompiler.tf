resource "azurerm_sentinel_alert_rule_scheduled" "potentially_suspicious_asp_net_compilation_via_aspnetcompiler" {
  ...
  description                = "Detects execution of \"aspnet_compiler.exe\" with potentially suspicious paths for compilation. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/potentially_suspicious_asp_net_compilation_via_aspnetcompiler.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
