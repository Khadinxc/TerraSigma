resource "azurerm_sentinel_alert_rule_scheduled" "potential_command_line_path_traversal_evasion_attempt" {
  ...
  description                = "Detects potential evasion or obfuscation attempts using bogus path traversal via the commandline - Google Drive - Citrix | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/potential_command_line_path_traversal_evasion_attempt.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
