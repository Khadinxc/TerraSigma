resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_file_characteristics_due_to_missing_fields" {
  ...
  description                = "Detects Executables in the Downloads folder without FileVersion,Description,Product,Company likely created with py2exe | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_file_characteristics_due_to_missing_fields.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
