resource "azurerm_sentinel_alert_rule_scheduled" "wab_execution_from_non_default_location" {
  ...
  description                = "Detects execution of wab.exe (Windows Contacts) and Wabmig.exe (Microsoft Address Book Import Tool) from non default locations as seen with bumblebee activity | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/wab_execution_from_non_default_location.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
