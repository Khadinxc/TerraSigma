resource "azurerm_sentinel_alert_rule_scheduled" "windows_admin_share_mount_via_net_exe" {
  ...
  description                = "Detects when an admin share is mounted using net.exe - Administrators | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/windows_admin_share_mount_via_net_exe.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
