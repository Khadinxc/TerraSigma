resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_registry_ie_security_zone_protocol_defaults_downgrade" {
  ...
  description                = <<DESC
    ...
    Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_registry_ie_security_zone_protocol_defaults_downgrade.yml
  DESC
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
