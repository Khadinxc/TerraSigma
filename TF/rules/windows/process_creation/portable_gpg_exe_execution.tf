resource "azurerm_sentinel_alert_rule_scheduled" "portable_gpg_exe_execution" {
  ...
  description                = "Detects the execution of \"gpg.exe\" from uncommon location. Often used by ransomware and loaders to decrypt/encrypt data. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/portable_gpg_exe_execution.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
