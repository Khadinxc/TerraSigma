resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_powershell_service_dacl_modification_set_service" {
  ...
  description                = <<DESC
    ...
    Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_powershell_service_dacl_modification_set_service.yml
  DESC
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
