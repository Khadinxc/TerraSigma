resource "azurerm_sentinel_alert_rule_scheduled" "rule_7zip_compressing_dump_files" {
  ...
  description                = "Detects execution of 7z in order to compress a file with a \".dmp\"/\".dump\" extension, which could be a step in a process of dump file exfiltration. - Legitimate use of 7z with a command line in which \".dmp\" or \".dump\" appears accidentally - Legitimate use of 7z to compress WER \".dmp\" files for troubleshooting | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/7zip_compressing_dump_files.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
