resource "azurerm_sentinel_alert_rule_scheduled" "copy_from_or_to_admin_share_or_sysvol_folder" {
  ...
  description                = "Detects a copy command or a copy utility execution to or from an Admin share or remote - Administrative scripts | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/copy_from_or_to_admin_share_or_sysvol_folder.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
