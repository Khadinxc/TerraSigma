resource "azurerm_sentinel_alert_rule_scheduled" "hiding_user_account_via_specialaccounts_registry_key_commandline" {
  ...
  description                = "Detects changes to the registry key \"HKLM\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon\\SpecialAccounts\\Userlist\" where the value is set to \"0\" in order to hide user account from being listed on the logon screen. - System administrator activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/hiding_user_account_via_specialaccounts_registry_key_commandline.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
