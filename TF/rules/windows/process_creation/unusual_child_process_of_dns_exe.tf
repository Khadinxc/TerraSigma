resource "azurerm_sentinel_alert_rule_scheduled" "unusual_child_process_of_dns_exe" {
  ...
  description                = "Detects an unexpected process spawning from dns.exe which may indicate activity related to remote code execution or other forms of exploitation as seen in CVE-2020-1350 (SigRed) | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/unusual_child_process_of_dns_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
