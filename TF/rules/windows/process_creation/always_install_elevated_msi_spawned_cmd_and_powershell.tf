resource "azurerm_sentinel_alert_rule_scheduled" "always_install_elevated_msi_spawned_cmd_and_powershell" {
  ...
  description                = "Detects Windows Installer service (msiexec.exe) spawning \"cmd\" or \"powershell\" | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/always_install_elevated_msi_spawned_cmd_and_powershell.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
