resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_scheduled_task_name_as_guid" {
  ...
  description                = "Detects creation of a scheduled task with a GUID like name - Legitimate software naming their tasks as GUIDs | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_scheduled_task_name_as_guid.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
