resource "azurerm_sentinel_alert_rule_scheduled" "code_execution_via_pcwutl_dll" {
  ...
  description                = "Detects launch of executable by calling the LaunchApplication function from pcwutl.dll library. - Use of Program Compatibility Troubleshooter Helper | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/code_execution_via_pcwutl_dll.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
