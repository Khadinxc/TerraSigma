resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_powershell_zip_compress" {
  ...
  description                = <<DESC
    ...
    Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_powershell_zip_compress.yml
  DESC
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
