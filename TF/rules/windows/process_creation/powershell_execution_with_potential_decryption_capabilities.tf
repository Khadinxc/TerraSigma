resource "azurerm_sentinel_alert_rule_scheduled" "powershell_execution_with_potential_decryption_capabilities" {
  ...
  description                = "Detects PowerShell commands that decrypt an \".LNK\" \"file to drop the next stage of the malware. - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/powershell_execution_with_potential_decryption_capabilities.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
