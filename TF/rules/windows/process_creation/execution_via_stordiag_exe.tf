resource "azurerm_sentinel_alert_rule_scheduled" "execution_via_stordiag_exe" {
  ...
  description                = "Detects the use of stordiag.exe to execute schtasks.exe systeminfo.exe and fltmc.exe - Legitimate usage of stordiag.exe. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/execution_via_stordiag_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
