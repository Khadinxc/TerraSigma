resource "azurerm_sentinel_alert_rule_scheduled" "set_suspicious_files_as_system_files_using_attrib_exe" {
  ...
  description                = "Detects the usage of attrib with the \"+s\" option to set scripts or executables located in suspicious locations as system files to hide them from users and make them unable to be deleted with simple rights. The rule limits the search to specific extensions and directories to avoid FPs | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/set_suspicious_files_as_system_files_using_attrib_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
