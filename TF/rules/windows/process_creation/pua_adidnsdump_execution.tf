resource "azurerm_sentinel_alert_rule_scheduled" "pua_adidnsdump_execution" {
  ...
  description                = "This tool enables enumeration and exporting of all DNS records in the zone for recon purposes of internal networks Python 3 and python.exe must be installed, Usee to Query/modify DNS records for Active Directory integrated DNS via LDAP | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/pua_adidnsdump_execution.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
