resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_download_from_file_sharing_website_via_bitsadmin" {
  ...
  description                = "Detects usage of bitsadmin downloading a file from a suspicious domain - Some legitimate apps use this, but limited. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_download_from_file_sharing_website_via_bitsadmin.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
