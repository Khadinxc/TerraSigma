resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_susp_ntfs_short_name_path_use_image" {
  name                       = "proc_creation_win_susp_ntfs_short_name_path_use_image"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Use Short Name Path in Image"
  description                = "Detect use of the Windows 8.3 short name. Which could be used as a method to avoid Image detection Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_susp_ntfs_short_name_path_use_image.yml - Applications could use this notation occasionally which might generate some false positives. In that case Investigate the parent and child process. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_susp_ntfs_short_name_path_use_image.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
