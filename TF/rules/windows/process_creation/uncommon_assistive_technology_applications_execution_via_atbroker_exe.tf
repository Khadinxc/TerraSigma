resource "azurerm_sentinel_alert_rule_scheduled" "uncommon_assistive_technology_applications_execution_via_atbroker_exe" {
  ...
  description                = "Detects the start of a non built-in assistive technology applications via \"Atbroker.EXE\". - Legitimate, non-default assistive technology applications execution | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/uncommon_assistive_technology_applications_execution_via_atbroker_exe.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
