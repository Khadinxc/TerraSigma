resource "azurerm_sentinel_alert_rule_scheduled" "windows_processes_suspicious_parent_directory" {
  ...
  description                = "Detect suspicious parent processes of well-known Windows processes - Some security products seem to spawn these | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/windows_processes_suspicious_parent_directory.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
