resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_hktl_krbrelayup" {
  name                       = "proc_creation_win_hktl_krbrelayup"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "HackTool - KrbRelayUp Execution"
  description                = "Detects KrbRelayUp used to perform a universal no-fix local privilege escalation in Windows domain environments where LDAP signing is not enforced Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_hktl_krbrelayup.yml - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_hktl_krbrelayup.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
