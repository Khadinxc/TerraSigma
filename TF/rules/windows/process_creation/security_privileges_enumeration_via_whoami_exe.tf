resource "azurerm_sentinel_alert_rule_scheduled" "security_privileges_enumeration_via_whoami_exe" {
  ...
  description                = "Detects a whoami.exe executed with the /priv command line flag instructing the tool to show all current user privileges. This is often used after a privilege escalation attempt. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/security_privileges_enumeration_via_whoami_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
