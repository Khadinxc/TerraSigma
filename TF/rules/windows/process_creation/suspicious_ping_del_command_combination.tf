resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_ping_del_command_combination" {
  ...
  description                = "Detects a method often used by ransomware. Which combines the \"ping\" to wait a couple of seconds and then \"del\" to delete the file in question. Its used to hide the file responsible for the initial infection for example | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_ping_del_command_combination.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
