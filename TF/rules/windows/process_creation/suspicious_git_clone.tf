resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_git_clone" {
  ...
  description                = "Detects execution of \"git\" in order to clone a remote repository that contain suspicious keywords which might be suspicious | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_git_clone.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
