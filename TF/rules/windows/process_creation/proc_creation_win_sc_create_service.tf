resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_sc_create_service" {
  name                       = "proc_creation_win_sc_create_service"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "New Service Creation Using Sc.EXE"
  description                = "Detects the creation of a new service using the \"sc.exe\" utility. Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_sc_create_service.yml - Legitimate administrator or user creates a service for legitimate reasons. - Software installation | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_sc_create_service.yml"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
