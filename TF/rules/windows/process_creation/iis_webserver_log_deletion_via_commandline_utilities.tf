resource "azurerm_sentinel_alert_rule_scheduled" "iis_webserver_log_deletion_via_commandline_utilities" {
  ...
  description                = "Detects attempts to delete Internet Information Services (IIS) log files via command line utilities, which is a common defense evasion technique used by attackers to cover their tracks. Threat actors often abuse vulnerabilities in web applications hosted on IIS servers to gain initial access and later delete IIS logs to evade detection. - Deletion of IIS logs that are older than a certain retention period as part of regular maintenance activities. - Legitimate schedule tasks or scripts that clean up log files regularly. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/iis_webserver_log_deletion_via_commandline_utilities.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
