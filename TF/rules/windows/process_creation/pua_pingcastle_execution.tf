resource "azurerm_sentinel_alert_rule_scheduled" "pua_pingcastle_execution" {
  name                       = "pua_pingcastle_execution"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "PUA - PingCastle Execution"
  description                = "Detects the execution of PingCastle, a tool designed to quickly assess the Active Directory security level. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/pua_pingcastle_execution.yml"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
