resource "azurerm_sentinel_alert_rule_scheduled" "pua_pingcastle_execution" {
  ...
  description                = "Detects the execution of PingCastle, a tool designed to quickly assess the Active Directory security level. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/pua_pingcastle_execution.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
