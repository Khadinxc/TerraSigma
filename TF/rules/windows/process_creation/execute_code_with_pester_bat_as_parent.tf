resource "azurerm_sentinel_alert_rule_scheduled" "execute_code_with_pester_bat_as_parent" {
  ...
  description                = "Detects code execution via Pester.bat (Pester - Powershell Modulte for testing) - Legitimate use of Pester for writing tests for Powershell scripts and modules | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/execute_code_with_pester_bat_as_parent.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
