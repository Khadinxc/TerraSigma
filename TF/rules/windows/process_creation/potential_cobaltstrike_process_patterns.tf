resource "azurerm_sentinel_alert_rule_scheduled" "potential_cobaltstrike_process_patterns" {
  ...
  description                = "Detects potential process patterns related to Cobalt Strike beacon activity | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/potential_cobaltstrike_process_patterns.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
