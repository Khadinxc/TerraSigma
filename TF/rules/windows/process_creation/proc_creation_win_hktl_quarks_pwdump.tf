resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_hktl_quarks_pwdump" {
  name                       = "proc_creation_win_hktl_quarks_pwdump"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "HackTool - Quarks PwDump Execution"
  description                = "Detects usage of the Quarks PwDump tool via commandline arguments Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_hktl_quarks_pwdump.yml - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_hktl_quarks_pwdump.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
