resource "azurerm_sentinel_alert_rule_scheduled" "interesting_service_enumeration_via_sc_exe" {
  name                       = "interesting_service_enumeration_via_sc_exe"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Interesting Service Enumeration Via Sc.EXE"
  description                = "Detects the enumeration and query of interesting and in some cases sensitive services on the system via \"sc.exe\". Attackers often try to enumerate the services currently running on a system in order to find different attack vectors. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/interesting_service_enumeration_via_sc_exe.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
