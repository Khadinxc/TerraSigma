resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_eventlog_clearing_or_configuration_change_activity" {
  ...
  description                = "Detects the clearing or configuration tampering of EventLog using utilities such as \"wevtutil\", \"powershell\" and \"wmic\". This technique were seen used by threat actors and ransomware strains in order to evade defenses. - Admin activity - Scripts and administrative tools used in the monitored environment - Maintenance activity | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_eventlog_clearing_or_configuration_change_activity.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
