resource "azurerm_sentinel_alert_rule_scheduled" "potential_psexec_remote_execution" {
  ...
  description                = "Detects potential psexec command that initiate execution on a remote systems via common commandline flags used by the utility | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/potential_psexec_remote_execution.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
