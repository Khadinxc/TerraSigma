resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_diantz_download_and_compress_into_a_cab_file" {
  ...
  description                = "Download and compress a remote file and store it in a cab file on local machine. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_diantz_download_and_compress_into_a_cab_file.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
