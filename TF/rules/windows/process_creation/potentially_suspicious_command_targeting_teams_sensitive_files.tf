resource "azurerm_sentinel_alert_rule_scheduled" "potentially_suspicious_command_targeting_teams_sensitive_files" {
  ...
  description                = "Detects a commandline containing references to the Microsoft Teams database or cookies files from a process other than Teams. The database might contain authentication tokens and other sensitive information about the logged in accounts. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/potentially_suspicious_command_targeting_teams_sensitive_files.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
