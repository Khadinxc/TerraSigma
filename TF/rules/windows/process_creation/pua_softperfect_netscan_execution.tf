resource "azurerm_sentinel_alert_rule_scheduled" "pua_softperfect_netscan_execution" {
  ...
  description                = "Detects usage of SoftPerfect's \"netscan.exe\". An application for scanning networks. It is actively used in-the-wild by threat actors to inspect and understand the network architecture of a victim. - Legitimate administrator activity | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/pua_softperfect_netscan_execution.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
