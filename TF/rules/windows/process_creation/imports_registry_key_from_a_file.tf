resource "azurerm_sentinel_alert_rule_scheduled" "imports_registry_key_from_a_file" {
  ...
  description                = "Detects the import of the specified file to the registry with regedit.exe. - Legitimate import of keys - Evernote | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/imports_registry_key_from_a_file.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
