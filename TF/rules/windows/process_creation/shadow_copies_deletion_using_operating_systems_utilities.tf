resource "azurerm_sentinel_alert_rule_scheduled" "shadow_copies_deletion_using_operating_systems_utilities" {
  ...
  description                = "Shadow Copies deletion using operating systems utilities - Legitimate Administrator deletes Shadow Copies using operating systems utilities for legitimate reason - LANDesk LDClient Ivanti-PSModule (PS EncodedCommand) | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/shadow_copies_deletion_using_operating_systems_utilities.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
