resource "azurerm_sentinel_alert_rule_scheduled" "renamed_jusched_exe_execution" {
  ...
  description                = "Detects the execution of a renamed \"jusched.exe\" as seen used by the cobalt group | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/renamed_jusched_exe_execution.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
