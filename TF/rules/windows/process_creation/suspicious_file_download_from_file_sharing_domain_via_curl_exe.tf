resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_file_download_from_file_sharing_domain_via_curl_exe" {
  ...
  description                = "Detects potentially suspicious file download from file sharing domains using curl.exe | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_file_download_from_file_sharing_domain_via_curl_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
