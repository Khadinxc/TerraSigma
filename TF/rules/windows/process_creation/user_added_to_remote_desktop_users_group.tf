resource "azurerm_sentinel_alert_rule_scheduled" "user_added_to_remote_desktop_users_group" {
  ...
  description                = "Detects addition of users to the local Remote Desktop Users group via \"Net\" or \"Add-LocalGroupMember\". - Administrative activity | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/user_added_to_remote_desktop_users_group.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
