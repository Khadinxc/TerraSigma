resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_msiexec_quiet_install_from_remote_location" {
  ...
  description                = "Detects usage of Msiexec.exe to install packages hosted remotely quietly | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_msiexec_quiet_install_from_remote_location.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
