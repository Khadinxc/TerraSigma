resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_dumpminitool_susp_execution" {
  name                       = "proc_creation_win_dumpminitool_susp_execution"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Suspicious DumpMinitool Execution"
  description                = "Detects suspicious ways to use the \"DumpMinitool.exe\" binary Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_dumpminitool_susp_execution.yml | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_dumpminitool_susp_execution.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
