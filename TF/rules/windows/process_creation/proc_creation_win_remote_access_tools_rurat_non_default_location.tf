resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_remote_access_tools_rurat_non_default_location" {
  ...
  description                = <<DESC
    ...
    Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_remote_access_tools_rurat_non_default_location.yml
  DESC
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
