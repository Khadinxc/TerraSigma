resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_winzip_password_compression" {
  ...
  description                = <<DESC
    ...
    Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_winzip_password_compression.yml
  DESC
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
