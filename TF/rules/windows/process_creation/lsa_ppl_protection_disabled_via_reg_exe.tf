resource "azurerm_sentinel_alert_rule_scheduled" "lsa_ppl_protection_disabled_via_reg_exe" {
  ...
  description                = "Detects the usage of the \"reg.exe\" utility to disable PPL protection on the LSA process - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/lsa_ppl_protection_disabled_via_reg_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
