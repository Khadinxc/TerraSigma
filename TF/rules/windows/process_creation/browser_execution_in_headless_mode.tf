resource "azurerm_sentinel_alert_rule_scheduled" "browser_execution_in_headless_mode" {
  ...
  description                = "Detects execution of Chromium based browser in headless mode | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/browser_execution_in_headless_mode.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
