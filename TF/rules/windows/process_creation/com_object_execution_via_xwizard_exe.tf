resource "azurerm_sentinel_alert_rule_scheduled" "com_object_execution_via_xwizard_exe" {
  ...
  description                = "Detects the execution of Xwizard tool with the \"RunWizard\" flag and a GUID like argument. This utility can be abused in order to run custom COM object created in the registry. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/com_object_execution_via_xwizard_exe.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
