resource "azurerm_sentinel_alert_rule_scheduled" "arbitrary_file_download_via_configsecuritypolicy_exe" {
  ...
  description                = "Detects the execution of \"ConfigSecurityPolicy.EXE\", a binary part of Windows Defender used to manage settings in Windows Defender. Users can configure different pilot collections for each of the co-management workloads. It can be abused by attackers in order to upload or download files. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/arbitrary_file_download_via_configsecuritypolicy_exe.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
