resource "azurerm_sentinel_alert_rule_scheduled" "windows_firewall_disabled_via_powershell" {
  ...
  description                = "Detects attempts to disable the Windows Firewall using PowerShell | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/windows_firewall_disabled_via_powershell.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
