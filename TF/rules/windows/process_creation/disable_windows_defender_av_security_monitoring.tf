resource "azurerm_sentinel_alert_rule_scheduled" "disable_windows_defender_av_security_monitoring" {
  ...
  description                = "Detects attackers attempting to disable Windows Defender using Powershell - Minimal, for some older versions of dev tools, such as pycharm, developers were known to sometimes disable Windows Defender to improve performance, but this generally is not considered a good security practice. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/disable_windows_defender_av_security_monitoring.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
