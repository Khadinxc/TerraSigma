resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_download_from_office_domain" {
  ...
  description                = "Detects suspicious ways to download files from Microsoft domains that are used to store attachments in Emails or OneNote documents - Scripts or tools that download attachments from these domains (OneNote, Outlook 365) | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_download_from_office_domain.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
