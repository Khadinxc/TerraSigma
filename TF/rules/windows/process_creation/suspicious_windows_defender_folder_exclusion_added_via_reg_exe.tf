resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_windows_defender_folder_exclusion_added_via_reg_exe" {
  ...
  description                = "Detects the usage of \"reg.exe\" to add Defender folder exclusions. Qbot has been seen using this technique to add exclusions for folders within AppData and ProgramData. - Legitimate use | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_windows_defender_folder_exclusion_added_via_reg_exe.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
