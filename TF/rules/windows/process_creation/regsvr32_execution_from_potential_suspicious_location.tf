resource "azurerm_sentinel_alert_rule_scheduled" "regsvr32_execution_from_potential_suspicious_location" {
  ...
  description                = "Detects execution of regsvr32 where the DLL is located in a potentially suspicious location. - Some installers might execute \"regsvr32\" with DLLs located in %TEMP% or in %PROGRAMDATA%. Apply additional filters if necessary. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/regsvr32_execution_from_potential_suspicious_location.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
