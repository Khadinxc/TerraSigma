resource "azurerm_sentinel_alert_rule_scheduled" "proc_creation_win_powershell_import_cert_susp_locations" {
  name                       = "proc_creation_win_powershell_import_cert_susp_locations"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Root Certificate Installed From Susp Locations"
  description                = "Adversaries may install a root certificate on a compromised system to avoid warnings when connecting to adversary controlled web servers. Reference: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_powershell_import_cert_susp_locations.yml - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/proc_creation_win_powershell_import_cert_susp_locations.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
