resource "azurerm_sentinel_alert_rule_scheduled" "dumping_of_sensitive_hives_via_reg_exe" {
  ...
  description                = "Detects the usage of \"reg.exe\" in order to dump sensitive registry hives. This includes SAM, SYSTEM and SECURITY hives. - Dumping hives for legitimate purpouse i.e. backup or forensic investigation | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/dumping_of_sensitive_hives_via_reg_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
