resource "azurerm_sentinel_alert_rule_scheduled" "uac_bypass_using_msconfig_token_modification_process" {
  ...
  description                = "Detects the pattern of UAC Bypass using a msconfig GUI hack (UACMe 55) | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/uac_bypass_using_msconfig_token_modification_process.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
