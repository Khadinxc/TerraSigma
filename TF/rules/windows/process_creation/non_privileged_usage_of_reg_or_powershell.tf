resource "azurerm_sentinel_alert_rule_scheduled" "non_privileged_usage_of_reg_or_powershell" {
  ...
  description                = "Search for usage of reg or Powershell by non-privileged users to modify service configuration in registry | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/non_privileged_usage_of_reg_or_powershell.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
