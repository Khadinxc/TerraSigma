resource "azurerm_sentinel_alert_rule_scheduled" "potential_persistence_via_vmwaretoolboxcmd_exe_vm_state_change_script" {
  ...
  description                = "Detects execution of the \"VMwareToolBoxCmd.exe\" with the \"script\" and \"set\" flag to setup a specific script to run for a specific VM state | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/potential_persistence_via_vmwaretoolboxcmd_exe_vm_state_change_script.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
