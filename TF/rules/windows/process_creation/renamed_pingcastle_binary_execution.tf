resource "azurerm_sentinel_alert_rule_scheduled" "renamed_pingcastle_binary_execution" {
  ...
  description                = "Detects the execution of a renamed \"PingCastle\" binary based on the PE metadata fields. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/renamed_pingcastle_binary_execution.yml"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
