resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_invoke_webrequest_execution" {
  ...
  description                = "Detects a suspicious call to Invoke-WebRequest cmdlet where the and output is located in a suspicious location | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/suspicious_invoke_webrequest_execution.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
