resource "azurerm_sentinel_alert_rule_scheduled" "deny_service_access_using_security_descriptor_tampering_via_sc_exe" {
  ...
  description                = "Detects suspicious DACL modifications to deny access to a service that affects critical trustees. This can be used to hide services or make them unstoppable. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/windows/process_creation/deny_service_access_using_security_descriptor_tampering_via_sc_exe.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
