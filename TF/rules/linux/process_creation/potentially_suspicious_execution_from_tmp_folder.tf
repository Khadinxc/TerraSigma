resource "azurerm_sentinel_alert_rule_scheduled" "potentially_suspicious_execution_from_tmp_folder" {
  ...
  query                      = <<QUERY
DeviceProcessEvents
| where FolderPath startswith "/tmp/" and (not(FolderPath endswith "/usr/bin/nextcloud"))
QUERY
