resource "azurerm_sentinel_alert_rule_scheduled" "python_reverse_shell_execution_via_pty_and_socket_modules" {
  ...
  description                = "Detects the execution of python with calls to the socket and pty module in order to connect and spawn a potential reverse shell. | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/python_reverse_shell_execution_via_pty_and_socket_modules.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
