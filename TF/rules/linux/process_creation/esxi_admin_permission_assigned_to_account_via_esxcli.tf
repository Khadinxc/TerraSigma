resource "azurerm_sentinel_alert_rule_scheduled" "esxi_admin_permission_assigned_to_account_via_esxcli" {
  ...
  description                = "Detects execution of the \"esxcli\" command with the \"system\" and \"permission\" flags in order to assign admin permissions to an account. - Legitimate administration activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/esxi_admin_permission_assigned_to_account_via_esxcli.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
