resource "azurerm_sentinel_alert_rule_scheduled" "triple_cross_ebpf_rootkit_install_commands" {
  ...
  description                = "Detects default install commands of the Triple Cross eBPF rootkit based on the \"deployer.sh\" script - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/triple_cross_ebpf_rootkit_install_commands.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
