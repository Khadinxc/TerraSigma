resource "azurerm_sentinel_alert_rule_scheduled" "scheduled_task_job_at" {
  ...
  description                = "Detects the use of at/atd which are utilities that are used to schedule tasks. They are often abused by adversaries to maintain persistence or to perform task scheduling for initial or recurring execution of malicious code - Legitimate administration activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/scheduled_task_job_at.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
