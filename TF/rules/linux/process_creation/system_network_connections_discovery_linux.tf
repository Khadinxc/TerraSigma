resource "azurerm_sentinel_alert_rule_scheduled" "system_network_connections_discovery_linux" {
  ...
  description                = "Detects usage of system utilities to discover system network connections - Legitimate activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/system_network_connections_discovery_linux.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
