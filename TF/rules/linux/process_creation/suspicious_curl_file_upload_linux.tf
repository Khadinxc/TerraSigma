resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_curl_file_upload_linux" {
  ...
  description                = "Detects a suspicious curl process start the adds a file to a web request - Scripts created by developers and admins | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/suspicious_curl_file_upload_linux.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
