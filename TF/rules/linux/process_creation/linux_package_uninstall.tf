resource "azurerm_sentinel_alert_rule_scheduled" "linux_package_uninstall" {
  ...
  description                = "Detects linux package removal using builtin tools such as \"yum\", \"apt\", \"apt-get\" or \"dpkg\". - Administrator or administrator scripts might delete packages for several reasons (debugging, troubleshooting). | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/linux_package_uninstall.tf"
  severity                   = "Low"
  query                      = <<QUERY
DeviceProcessEvents
