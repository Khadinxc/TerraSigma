resource "azurerm_sentinel_alert_rule_scheduled" "linux_base64_encoded_pipe_to_shell" {
  name                       = "linux_base64_encoded_pipe_to_shell"
  log_analytics_workspace_id = var.workspace_id
  display_name               = "Linux Base64 Encoded Pipe to Shell"
  description                = "Detects suspicious process command line that uses base64 encoded input for execution with a shell - Legitimate administration activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/linux_base64_encoded_pipe_to_shell.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
