resource "azurerm_sentinel_alert_rule_scheduled" "linux_base64_encoded_shebang_in_cli" {
  ...
  description                = "Detects the presence of a base64 version of the shebang in the commandline, which could indicate a malicious payload about to be decoded - Legitimate administration activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/linux_base64_encoded_shebang_in_cli.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
