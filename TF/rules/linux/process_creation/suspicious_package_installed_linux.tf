resource "azurerm_sentinel_alert_rule_scheduled" "suspicious_package_installed_linux" {
  ...
  description                = "Detects installation of suspicious packages using system installation utilities - Legitimate administration activities | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/suspicious_package_installed_linux.tf"
  severity                   = "Medium"
  query                      = <<QUERY
DeviceProcessEvents
