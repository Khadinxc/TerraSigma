resource "azurerm_sentinel_alert_rule_scheduled" "shell_execution_via_rsync_linux" {
  ...
  description                = "Detects the use of the \"rsync\" utility to execute a shell. Such behavior may be associated with privilege escalation, unauthorized command execution, or to break out from restricted environments. - Legitimate cases in which \"rsync\" is used to execute a shell | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/process_creation/shell_execution_via_rsync_linux.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceProcessEvents
