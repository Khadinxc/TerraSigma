resource "azurerm_sentinel_alert_rule_scheduled" "triple_cross_ebpf_rootkit_default_persistence" {
  ...
  description                = "Detects the creation of \"ebpfbackdoor\" files in both \"cron.d\" and \"sudoers.d\" directories. Which both are related to the TripleCross persistence method - Unlikely | Source: https://github.com/SigmaHQ/sigma/blob/master/rules/linux/file_event/triple_cross_ebpf_rootkit_default_persistence.tf"
  severity                   = "High"
  query                      = <<QUERY
DeviceFileEvents
